...
LocalDB.prototype.open = function (callback) {
    ...
    // 获取一条数据
    LocalDB.prototype.get = function (id, callack) {
        var objStore = this.getStore()
        var request = objStore.get(id)
        request.onsuccess = function (event) {
            callack && callack(event.target.result)
        }
    }
    ...
    LocalDB.prototype.getAll = function (callback) {
        ...
        request.onsuccess = function (event) {
            var cousor = event.target.result
            if (cousor) {
                // 如果油表存在，执行回调并传入当前数据行
                callback && callback(cousor.value)
                // 继续下一行数据
                cousor.continue()
            }
        }
    ...
    LocalDB.prototype.remove = function (id) {
        var objStore = this.getStore()
        var result = objStore.delete(parseInt(id))
    }
}